"""Additive migration and audit for Task37-B05 production gates."""
...
CONFIRMATION = "APPLY_TASK37_B05_PRODUCTION_GATE"
    ...
    with get_connection() as conn:
        ...
        columns = {
            ...
            for row in list_database_columns(
                conn, "therapeutic_assessment_release_evidence"
            )
        } if "therapeutic_assessment_release_evidence" in tables else set()
    return {
        "ok": TABLES.issubset(tables) and EVIDENCE_COLUMNS.issubset(columns),
        "schema_version": CURRENT_SCHEMA_VERSION,
        "schema_name": CURRENT_SCHEMA_NAME,
        "missing_tables": sorted(TABLES - tables),
        "missing_evidence_columns": sorted(EVIDENCE_COLUMNS - columns),
    }
