...
记录控制页：强度数字与量尺为主角，加减保持大触控，补充维度与输入分区；输入增加可见标签与计数尾行，不用占位长句挤压单行。保留全部17个页面事件、值范围、保存/回执、曲线点选与API。
...
旧触摸计算使用1+ratio*9，指针却用value/10，导致刻度与触点不一致；本轮仅把显示百分比统一为(value-1)/9，提交值与API不变。曲线坐标文字从10px提升为13px，不改数据、坐标点或命中算法。
