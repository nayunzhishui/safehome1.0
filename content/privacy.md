...
我们通过小程序内隐私入口接收申请。联系渠道、处理主体发生变化时，将更新本政策并说明影响；不使用源码文件路径代替用户可阅读的政策。
...
## 四、记录、测评与练习数据
按你选择的功能，我们处理目标、具体事件、情绪与回应、测评作答和结果、练习及打卡、阶段复盘等内容，用于保存、回看及相应支持性反馈。
...
## 十、查阅、更正、撤回与删除
你可以通过小程序内隐私入口或本政策列明的联系渠道提出请求。处理前可能核验账号及请求范围，并提供处理状态。
...
## 十一、安全措施
用户访问云托管服务的网络链路使用平台提供的安全连接能力，并结合账号认证、权限控制、审计、备份与异常处理保护数据。
当前部署的特定同环境内网数据库链路采用既有受限连接策略，不将其描述为所有链路均启用TLS。内网例外不适用于公网数据库访问，也不能替代持续的风险评估与改进。
