...
function Assert-PackageSourceMatchesCommit {
  ...
  try {
    & git -C $Root archive --format=zip "--output=$referenceArchive" $Commit Dockerfile .dockerignore backend content shared config/rc0810/database_profiles.json config/rc0810/database_recovery_policy.json deploy/verify_rc0810_f03_images.py
...
try {
  ...
  if ($manifestText -notmatch "Included=Dockerfile,.dockerignore,backend,content,shared,config/rc0810/database_profiles.json,config/rc0810/database_recovery_policy.json,deploy/verify_rc0810_f03_images.py") {
