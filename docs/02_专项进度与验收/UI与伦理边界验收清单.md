...
进度口径：本文用于试点前逐页验收。当前学生画像、沙盘表达、高风险复核和画像导出仍为规划检查项，不代表相关页面和接口已经实现。最新状态见 `docs/00_当前事实基准/项目进度统一口径.md`。
...
| 支持性反馈 |  |  |  |  | 检查反馈是否支持性、是否知道下一步 |
| 训练中心 |  |  |  |  | 检查卡片分类和推荐入口 |
| 训练卡详情 |  |  |  |  | 检查步骤是否可执行，按钮是否明显；内容库需保留理论来源、禁用情境和复盘问题 |
| 打卡 |  |  |  |  | 检查是否像追踪，不像评分 |
| 周报 |  |  |  |  | 检查是否像复盘，不像成绩单 |
| 个人中心 |  |  |  |  | 检查入口清楚，信息不过度拥挤 |
| 人工督导 |  |  |  |  | 检查非实时、非危机边界 |
