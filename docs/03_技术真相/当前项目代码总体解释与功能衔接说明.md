...
## 2026-08-25：F16 数据库安全与隔离恢复链路

production MySQL 配置先由 `Config.validate()` 检查 CA、主机身份校验和最低 TLSv1.2；连接池与直接连接都使用同一 SSLContext。`database_recovery_policy.json` 冻结关键 owner 关系及孤儿检测合同。备份/恢复服务仅执行合成 SQLite 演练：备份清单绑定 migration head、计数和摘要，恢复目标必须有 `isolated_validation` 标识，恢复后再次核对完整性、关系、计数和抽样摘要。该演练不能替代真实 MySQL 恢复、生产权限核对或 RPO/RTO 签字。
...
## 附录 C：RC0810-F15 主动安全时钟

`backend/services/safety_scheduler_service.py` 不依赖用户读取页面来推进超时。外部 cron 或 worker 每分钟调用 `backend/scripts/run_safety_scheduler.py`，使用 UTC 时间和数据库全局租约，只允许一个 worker 扫描：

```text
risk_review_records 到期 -> priority_review + urgent + 唯一升级事件
therapeutic_assessment_work_queue 到期 -> handoff_required + 队列运行态刷新
therapeutic_assessment_safety_events 超时/无责任链 -> safety_paused + kill switch
```

无人值守时停止自动反馈、自由文本 AI 和治疗性新受理；低风险记录保存、人工支持状态与隐私权利保持可用。暂停后恢复必须有真人证据，并设置补扫标记。工程测试没有证明真实人员容量，production 仍关闭。
