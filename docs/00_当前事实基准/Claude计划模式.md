...
| T19-03 研究者消息闭环 | 自动开发完成 | `POST /api/messages`、分页/已读/幂等/发送者字段及工作台表单 | 关系试点消息闭环测试通过 | 云端真实账号与真机收取 |
...
| T19-06 阶段性反馈与仪表盘 | 自动开发完成 | 报告PATCH新版本、阶段反馈消息、时间轴、第4步与仪表盘层级 | 关系试点和小程序契约通过 | 研究者真实流程验收 |
| T19-07 验证与收口 | 自动验证完成 | API/数据库/事实文档同步 | 内容、专项、小程序、Web通过；后端285 passed、1个既有周报用例deselected | CloudBase 发布 |
