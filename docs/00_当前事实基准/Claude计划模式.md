...
### 2026-09-09 全53路由实现 checkpoint（当前）

- [x] 前46页结构/命名/语法、78WXML/71WXSS与已有专项复查；Figma主稿温度计10个小刻度调至13px。
- [x] 余7页先Figma再前端，当前53/53（51业务+2开发）implemented_pending_final_check，不是验收通过。
- [x] 人工支持关联优先级/读取错误重试修正；Luna同一reviewer发现手动不关联被重试覆盖，修复后18项专项通过，复审pass。
- [x] 依用户补充校准字号、长内容与返回按钮；26个合成排版场景通过。最后一版全53页85WXML/78WXSS原生编译通过。
- [ ] 统一完整Figma状态与原生视觉对照、真机/字号放大/键盘/读屏/返回栈验收。
- [ ] 通用用途授权UI、新手完成/跳过状态按真实合同继续，不用说明页冒充授权，不自动同意。
- [ ] 不自动合并其他分支、推送、发布；独立动画保持不动。下一阶段是统一验收准备，不继续扩展无关页面。

### 2026-09-09 第39–46页 checkpoint（历史）

- [x] 按顺序完成8页Figma主稿与前端，进度46/53；无ImageGen。原题、计分、API、数据库、权限服务不改。
- [x] 修正0强度/正文截断/未知数量/收藏语义/来源限制/空表登录恢复；补高风险普通推荐、旧派生缓存与直接跳转防护，安全入口前置。
- [x] 新手知情/隐私/保护导航、AI暂不使用返回、学生本人待决定保护Figma配套完成；无自动同意，不虚构通用授权状态。
- [x] 23 WXML/24 WXSS编译，11项Node+3项既有结果布局测试，8页原行为属性和118次画布调用一致；37个合成排版场景通过。
- [x] Luna同一reviewer两项finding已修复并复审pass；仅关键代码增量通过。
- [ ] 完整Figma/原生运行/真机/读屏/键盘未验收。
- [ ] 通用用途授权记录UI与首次使用完成/跳过状态尚未接入，继续前先核对真实合同；不是本次新增后台功能。
- [ ] 下一候选第47页task-detail，剩余7页；本轮不自动超出8页，不推送、不发布、不碰独立动画。

### 2026-09-09 第33–38页定向审查 checkpoint（历史）

- [x] 接续上轮6页未提交改动，先核对真值/冻结，再修正Figma和前端。无ImageGen，无API/后端/页面业务JS变更。
- [x] 目标15选项与输入、课程固定0假统计、设置无效类名/假聚合稿、个人账号与开放列表、课程阅读层级、研究导航及原生条件循环问题已定向修正。
- [x] 课程与个人页增加静态非交互弧纹；标题装饰变量只在六页启用，组件旧默认保留。
- [x] 13 WXML/14 WXSS原生编译、6页事件/输入/JS/JSON、组件默认/转发及26个合成浏览器排版场景通过；Figma目标与隐私按钮截图复查完成。
- [ ] 38/53仅implemented_pending_final_check / review_pending_wave；研究者其它工作区、保护Figma变体、全部原生状态及键盘/读屏/大字体/真机待验收。
- [ ] 本轮到六页复查为止，不自动继续第39页；下一候选pages/diary-form/index，按用户后续指令执行。未推送/发布，独立动画不入提交。

### 连续实现与审美校准 checkpoint（2026-09-08历史）

- [x] app.json前32/53页已实现；本轮第20–32页覆盖共同理解总览、八个分步页、行动回看、质量与更正、成长总览、关系探索手记，节点见figma-state.uiProduct2。
- [x] 先核对功能/冻结，再逐页Figma及前端；总览主次行动、政策边界、步骤说明、来源区分、输入与自愿确认保持真实。
- [x] 仅新增展示ui2属性，流程工厂JS及API完全未改；关系手记行为合同、1 WXML/2 WXSS原生编译、任务显示映射和动效降级检查通过。
- [x] 修正Figma旧组件尺寸/绝对定位覆盖失败、标签断行、空日期控件高度和遗漏政策边界；使用最少UI2变体，不扩大组件库。
- [ ] 全部为implemented_pending_final_check / review_pending_wave；剩余21页，复杂状态、Figma/代码完整对照与真机未验收。
- [ ] 下一页pages/researcher-dashboard/index（第33页），随后其余路由。
...
- [x] 06–32 已按当前checkpoint实施；并非最终验收通过。
- [ ] 33–53：按app.json其余路由顺序继续，每页先Figma后代码；不将统一主题命中视为该页已重构。
