...
测试必须覆盖真实受影响范围，但不得在同一源码快照机械重复。

每次运行检查前，先说明它会发现哪种具体失败，以及失败后会采取什么不同动作；答不上来就不运行。

按以下层级执行：

1. 开发中只运行首个失败合同、当前任务专项和直接受影响测试。
2. 代码与机器合同冻结后，运行一次受影响组合回归；组合套件已包含专项时，不再单独重复专项，测试数按唯一用例记录。
3. 修复审查 finding 时，先运行最小复现、finding 对应测试和直接受影响回归；全部 finding 关闭后再运行一次冻结层回归。
4. 公共认证/授权、数据库、迁移、API/shared 契约、基础配置、内容规则或生产构建变化，才触发相应跨模块回归。
5. 纯说明性 Markdown、结论回填或排版变化只运行文档一致性与 `git diff --check`；文档若是机器输入、发布材料或安全/隐私合同，仍运行其专用验证器。
6. 同一源码、测试、依赖、配置、命令和环境已经成功的证据可以在同一任务中复用；不得只因生成新摘要、回填 decision 或再次审查而重跑。
7. 全量测试只在阶段/波次收口、required CI、最终 RC、用户明确要求，或真实跨域影响无法由受影响测试覆盖时运行。

不得用分层回归跳过权限、隐私、迁移、生产制品、数据完整性或发布门禁。发现真实跨层影响时立即扩大到对应回归，不扩大到无依赖关系的测试。

### 5.2 Subagent 精简规则

Subagent 用于真正需要独立判断的任务，不用于代替主智能体执行机械工作。

1. 默认由主智能体完成实现、测试执行、文档整理和普通 Fix Loop。
2. 一个连续计划固定使用一个 reviewer，并按计划约定的少量波次集中审查，不按每个小任务新建 subagent。RC0810 后续固定三个审查波次：F07 至 F12-B、F13 至 F21、F22-B 至 F26。
3. 首次启动 reviewer 时使用 `fork_turns="none"`。后续使用 `followup_task` 向同一 reviewer 发送增量；只有 reviewer 已结束且无法恢复时，才允许登记原因和最后 checkpoint 后替换，任何时刻只能有一个 reviewer。
4. reviewer 只在波次冻结点接收精简审查包：checkpoint base、波次实际 diff、验收标准、开放风险、测试证据、迁移/回滚结果和直接相关文档；不得继承整段长对话或重读无关历史。
5. reviewer 复算已有证据并审查真实风险，只重跑专项、finding 复现或证据缺口用例；源码和证据未变化时不重复长时间组合回归。
6. `fix_required` 后继续使用同一个 reviewer，只发送 finding、修复差异、新 packet 和相关测试结果；不得为同一 finding 或同一波次新建 subagent。
7. 波次内小任务由主智能体自审并独立提交，状态记为 `review_pending_wave`；波次审查通过后才标记 `review_pass`。已独立审查通过且输入未变的历史任务不重复审查。
8. 权限、Consent、隐私、迁移、生产制品、风险处置、安全 Gate 和最终 RC 必须进入对应波次独立审查。若下一步将执行不可逆外部动作、真实数据迁移或生产发布，必须在该动作前提前审查当前增量。
9. review pass 后只回填 decision、commit、push 等事实时，不再次启动 subagent；若回填改变需求、机器合同、代码或验收标准，则重新冻结并审查。

Subagent 不负责等待长测试或重复报告“仍在运行”。长测试由主智能体或 Harness 执行，reviewer 使用绑定证据判断是否需要定向复验。
